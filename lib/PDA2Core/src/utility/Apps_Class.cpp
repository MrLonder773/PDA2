...
void Apps_Class::showToast(const char* text, uint32_t ms) {
    ...
    lv_obj_move_foreground(_toast_cont);
...
void Apps_Class::_build_launcher() {
    ...
    lv_label_set_text(lbl_time, "PDA 2");
    lv_obj_set_style_text_color(lbl_time, lv_color_hex(0xaaaaaa), 0);
    lv_obj_set_style_text_font(lbl_time, &lv_font_montserrat_16, 0);
    lv_obj_set_style_bg_opa(lbl_time, LV_OPA_COVER, 0);
    lv_obj_set_style_bg_color(lbl_time, lv_color_hex(0x1e2235), 0);
    ...
    lv_obj_set_style_bg_color(_toast_cont, lv_color_hex(0x222222), 0);
    lv_obj_set_style_bg_opa(_toast_cont, LV_OPA_90, 0);
